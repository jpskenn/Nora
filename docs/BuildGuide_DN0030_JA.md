...
1. スペーサーシートをはさみなどで切り離し、スイッチプレート裏側の貼り付け位置へ、以下の優先順に貼り付けます。
...
1. 光らないLEDを新しいものに交換する
    ...
    - 熱々にして持ち上げる方法
      溶けたはんだで火傷しないように注意します。
        1. こて先の温度を320℃程度にします。
        1. LEDの端子部分を温め、はんだをたーーーーーーっぷりと溶かし、端子部分につけておきます。（あずき〜大豆くらいの大きさの溶けたはんだ）
...
１． [QMK Toolbox](https://github.com/qmk/qmk_toolbox/releases)をダウンロードし、起動します。
...
- レイヤーインジケータLED
  左上2個（1行目の左から2個）をレイヤーインジケータLEDとして使用し、レイヤーの状態を表示します。
