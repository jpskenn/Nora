...
”Fearless Wings（以下FWと呼称）”と”Fearless Wings + Rotary Encoder（以下FW+REと呼称）”の、ふたつのレイアウトの組み立て方法について説明します。
...
| ソケット, Kailh Chocロープロファイル用 | 63〜64 | FW：64個, FW+RE 63個 |
| スイッチ, Kailh Choc v1 | 63〜64 | FW：64個, FW+RE 63個 |
| キーキャップ, Choc用<br>MBKまたはそれ以下のサイズのもの | 63〜64 | FW：64個（1u 56個, 1.5u 8個）<br>FW+RE 63個（1u 54個, 1.5u 8個, 1u 1.25u 1.5uのうち、いずれか1個 |
| ロータリーエンコーダ<br>ALPS EC11, EC12または同等品 | 1 | FW+REを組み立てる場合のみ |
| USBケーブル （Type-C） | 1 | |
...
| 部品名 | 個数 | 備考 |
| ----- | :-----: | ----- |
| LED<br>SK6812MINI-E | 64〜65 | FW 64個<br>FW+RE 65個（設計ミスのためDN0031では65個必要） |
...
**注意**
- 行ごとに、LEDを取り付ける方向が異なります。
- レイアウトによってジャンパする位置と、LEDの取り付け箇所が異なります。
