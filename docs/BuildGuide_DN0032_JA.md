# Nora, Fearless Wings v.1(DN0032) ビルドガイド
...
- EEPROM搭載で、多くのレイヤーを使用できる
- ボトムプレート以外にも、GH60型，Poker型のロープロファイルケースに取り付けできる
- キーバックライトLED（オプション）
...
- スイッチの取り付け
  ...
  （キーキャップは、通常の方向に問題なく取り付けることができます。）  
...
- ソケットの取り付け
  ...
  GH60型，Poker型のロープロファイルケースへの取り付けなども含め、実用上の問題がないことを確認済みです。  
...
![ジャンパの位置](../assets/BuildGuide_DN0032/PCB_LED_jumper.jpeg)
ジャンパ`JP4`から`JP9`の位置
...
<img src="../assets/BuildGuide_DN0032/PCB_socket_painted.jpeg" width="300">

ソケット取り付け指標（白く塗られた箇所）

<img src="../assets/BuildGuide_DN0032/PCB_socket_framed.jpeg" width="300">

ソケット取り付け指標（外枠のみ描かれた箇所）
...
1. はんだ付けした状態を確認します。  
    ...
    ![ソケットをはんだ付けした状態](../assets/BuildGuide_DN0032/DSC_7775.jpeg)  
    ソケットをはんだ付けした状態
...
- 白い線で描かれた貼り付け位置が埋まる程度に貼り付けてあれば、シートが多少余っていても問題ありません。  
  余ったシートは空いている箇所に追加で貼り付けたり、ボトムプレートへ貼り付けて基板の振動を低減したりするのもよいでしょう。  
  →[打鍵感向上：ケース内部の制振と静音化](#打鍵感向上ケース内部の制振と静音化)
...
- 基板の振動防止
    ...
    ![スペーサーシート貼り付け位置](../assets/BuildGuide_DN0032/IMG_4305.jpeg)  
    ボトムプレートへ、余ったスペーサーシートを貼り付けた例
...
ジャンパし忘れや、ジャンパする箇所を間違えていることが原因のひとつと考えられます。

[LEDの配線経路をジャンパする](#ledの配線経路をジャンパする)の通りにジャンパされているか確認します。
...
各レイアウトに対応するChoc V1用キーキャップと、使用感などをまとめました。
...
| :---- | :---: | :---: | :---: | :---: | :---- |
Kailhロープロ刻印 | セット | ○ | ◎ | ◎ | *キーの寸法が少し大きいため、隣同士のキーが当たるかもしれません。<br>ソケットを傾かないように取り付けたり、キーキャップの成形バリを取り除いたりすると問題なく使用できます。*<br><br>- 刻印あり<br>- バックライトLED対応 透過タイプ<br>- 低価格<br><br>キー天面は平らな形状で、4辺が斜めにカットされています。<br>Fullレイアウトでは1.5uのキーが足りないため、最下行の中央には1.25uまたは1uを取り付けます。<br>スペースキー用にKailhロープロ無刻印などの単品キーを追加すると、見た目が良くなります。 |
| Kailhロープロ無刻印 1u, 1.5u<br>(黒, 白, 無塗装） | 単品 | ◎ | ◎ | ◎ | *キーの寸法が少し大きいため、隣同士のキーが当たるかもしれません。<br>ソケットを傾かないように取り付けたり、キーキャップの成形バリを取り除いたりすると問題なく使用できます。*<br><br>- 刻印なし<br><br>同じKailhロープロの刻印ありと組み合わせても、無刻印を単体で使用してもどちらも良い感じの見た目になります。<br>無塗装は少しクリーム色っぽい乳白半透明のような感じで、バックライトが綺麗に光ります。 |
| Kailhロープロ無刻印 1u<br>（クリア） | 単品 | × | × | × | クリアのみ形状が異なり、キーのサイズが少し大きくChoc用キーピッチに対応しないため、取り付けできません。 |
| MBK Legend Glow | セット | ◎ | ◎ | ◎ | - 刻印あり<br>- バックライトLED対応 透過タイプ<br><br>少し高価ですが、1uと1.5uのブランクキーが豊富に付属し、1.5uのコンベックスタイプも2個も用意されているのが嬉しいです。<br><br>キー天面はスフェリカル（球状にえぐれた）な形状です。<br>スイッチへの抜き差しが少しきついかも。 |
| MBK 1u, 1.5u| 単品 | ◎ | ◎ | ◎ | - 刻印なし<br><br>これまでは黒と白だけのラインナップでしたが、カラー版も販売されるようになりました。<br>Modとアルファ部でツートーンにしたり、アクセントカラーを入れたりして色々と楽しむことができます。<br>また白いキーは染色に適しており、オリジナルの色に染めることもできます。 |
| Gravity Keycaps | セット | △ | △ | △ | - 刻印なし<br><br>Choc V1用キーキャップとしては唯一（？）の、キーの位置に合わせて個別のキー形状が与えられたChoc用キーキャップで、打鍵のしやすさはピカイチ。<br><br>40キー程度のキーボード用として作られているため、Noraで使用するには多くのキーが足りません。<br>MBKの黒色とのマッチングが良好なので、これと組み合わせるとよいでしょう。 |
| WRK | セット | × | × | × | MX用の19.05*19.05mmピッチを基準に作られているため、取り付けできません。 |
| MCC-profile POM Choc keycap 1u | 単品 | △ | △ | △ | - 刻印なし<br><br>キー天面はシリンドリカル（中央がU字型に凹む）な形状です。<br>POM素材がとてもスベスベで、汚れがつかず、ずっと真っ白なまま使えるのも嬉しいです。<br><br>1uサイズしかラインナップされていないため、1.5uは他のキーキャップを使用します。<br>MBKの白がMCCに近い白色ですが、MBKはキーバックライトを透過しません。<br>バックライトを使う場合は、Kailhロープロ無刻印の無塗装と組み合わせるのがおすすめです。 |
...
- レイヤー
    ...
    | :-: | :-- | :-- | :-: |
    | 0 | ベースレイヤー #1 | 日本語配列 | 未割り当て<br>（RGB Matrixのエフェクト色） |
    | 1 | ベースレイヤー #2 | 英語配列 | 青, 青 |
    | 2 | ベースレイヤー #3 | テンキー | 黄, 黄 |
    | 3 | Lower #1 | ファンクションキー<br>記号（日本語配列用）<br>オーディオコントロール | 緑, 緑 |
    | 4 | Lower #2 | ファンクションキー<br>記号（英語配列用）<br>ボリューム | 明るい緑, 明るい緑 |
    | 5 | Lower #3 | 未設定 | 未割り当て<br>（RGB Matrixのエフェクト色） |
    | 6 | Raise #1 | ファンクションキー<br>数字<br>カーソル移動 | 明るい青, 明るい青 |
    | 7 | Raise #2 | ファンクションキー<br>数字<br>カーソル移動 | シアン, シアン |
    | 8 | Raise #3 | 未設定 | 未割り当て<br>（RGB Matrixのエフェクト色） |
    | 9 | Adjust | ベースレイヤー切り替え<br>LED等の設定<br>スクロールロックなど | 赤, 赤 |
...
1. （LEDを使用する場合）LEDの配線経路ジャンパを変更します。
  レイアウトに合わせて、ジャンパを変更します。
  'Full'に変更した場合は、はんだを取り除いて、隣同士のパッドがつながらないようにします。

1. （LEDを使用する場合）LEDを追加で取り付け、または余分なLEDを取り外します。
...
- キーマトリクスを駆動して、キーボードとして使用する
- LEDを点灯させる（16個までの点灯を確認済み）

#### できるかもしれないこと

以下は、動作未確認の項目です。

- すべてのLEDの点灯（USBの消費電力に注意が必要）
...
![Pi PICO wiring](../assets/BuildGuide_DN0032/PCB_pico_schematic.png)

#### 取り付け方法

1. 基板裏側の中央上部へ、ピンヘッダなどを使ってPi PICOを取り付けます。

    ![Pi PICO 取り付け箇所](../assets/BuildGuide_DN0032/PCB_pico_position.png)  
    Pi PICO 取り付け箇所と、電源供給元ジャンパの位置

    ピンヘッダやピンソケット、コンスルーなどがスイッチ用ソケットと干渉しないように、  
    高さ：2mm以上  
    幅：2.5mm以下  
    のものを使用します。

    ![Pi PICOを取り付けた状態](../assets/BuildGuide_DN0032/IMG_4289.jpeg)  
    Pi PICOを取り付けた状態

    なお、基板とスイッチプレートの間隔が1mmしかないため、コンスルーが奥まで刺さり切らない場合があります。  
    ピンヘッダやピンソケットは、基板へはんだ付けした後に飛び出したピンをニッパーなどでカットして1mm以下にします。

1. LEDとEEPROMへの電源供給元ジャンパを、`PICO VBUS`へ切り替えます。
    ...
    | オープン | 接続なし | PICO使用時に、LEDやEEPROMへ電源供給したくない場合 |

1. （オプション）必要に応じて、I2Cプルアップ抵抗への接続を切り替えます。
...
1. （オプション）ボトムプレートの一部を切り離します。

    ボトムプレートと組み合わせる場合は、プレート中央部を切り離します。  
    左右につながっている4箇所をニッパーなどでカットしてから、マウスバイト（点線部分）に沿って折り曲げると切り離せます。  
    切り離した部分が尖っているため、やすりなどを使って滑らかにします。

    ![ボトムプレートの一部を切り離した状態](../assets/BuildGuide_DN0032/IMG_4292.jpeg)  
    ボトムプレートの一部を切り離した状態

#### ファームウェア

ファームウェアに[PRK Firmware](https://github.com/picoruby/prk_firmware)を使用して、動作を確認済みです。

- 全キーの入力
- LED点灯（16個まで）

動作確認に使用したファームウェアはこちら：[keymap.rb](../Firmware/prk_nora/keymap.rb)
