...
- GH60型, Poker型ケースを使用する場合、スイッチとケースのネジ穴との干渉を避けるため、スイッチの足の加工が必要です。
...
| Mill-Maxソケット 銀色| 4 | [<img alt="" src="../assets/BuildGuide_v.0.4/_DSF0432_3_s.jpeg" width="300">](../assets/BuildGuide_v.0.4/_DSF0432_3.jpeg)<br>はんだ付け練習用 |
...
| MBKキーキャップ | 必要数 | 左シフト2u：<br>1u 65個, 1.5u 6個, 2u 3個<br>左シフト1u+1u：<br>1u 67個, 1.5u 6個, 2u 2個 |
...
キースイッチまたはMill-Maxソケットをはんだ付けするスイッチ端子スルーホール（基板上の穴）が、電気的に正しく反応することを確認します。  
...
ピンセットなどの電気を通すものを使い、スイッチ端子スルーホールをショートさせます。
...
スイッチ端子スルーホールをショートさせ、全てのキーの色が変わることを確認します。  
...
LEDの発光部分（透明っぽい窓があるところ）が下側になるように持ちます。
...
1、2：4本の端子のどれかひとつにこて先をあてて押さえ、端子とパッド（端子がはんだ付けされる、基板の銀色部分）を温めます。  

3、4：はんだをこて先に当てて溶かし、端子とパッドにはんだを行き渡らせて、はんだを離します。（こて先は当てたまま動かしません）

5：こて先を離します。

はんだが、端子とパッド全体に行き渡っていれば良い仕上がりです。  
LEDが傾いてしまった場合は、ピンセットでLEDをつまみ、はんだ付けした端子をこて先で温めながら修正します。  

同様の手順で、最初にはんだ付けした端子とLED本体を挟んだ反対側の端子を、1本ずつはんだ付けします。

残った最後の1本の端子をはんだ付けします。
...
ケースネジの上に取り付けるスイッチは、スイッチ端子のスルーホールに直接はんだ付けせず、Mill-Maxソケットを介して取り付けます。  
スイッチ端子のスルーホールに`Socket`と書かれた箇所へ、基板おもて側からMill-Maxソケットをはんだ付けします。
...
中央下部、スペースキー付近にはんだ付け練習用のスルーホールを用意しています。  
はんだ付けに不安がある場合は、いくつか練習してから本番に挑みましょう。

〓写真　練習用の場所

`Socket`と書かれた箇所へ、基板おもて側からMill-Maxソケットを差し込みます。  
基板を裏返したときにポロッと外れてしまうようなら、マスキングテープでとめておきます。

基板の裏側から、こて先をMill-Maxソケットとスルーホールの両方にあて、1、2、3と数えて温めます。  

〓写真　こて先をあてている状態

そこへはんだを少しだけ押し付けてはんだを溶かし、ソケットとスルーホールの間へ流し込みます。  
ソケットの内側にはんだが入ってしまわないよう、少しずつおこないます。

はんだがスルーホールから少しだけ出ている程度まで流し込んで完了とします。
...
基板とスイッチプレートの隙間を埋めるスポンジ状のシートを、スイッチプレート裏側に貼り付けます。  
シートをはさみなどで切り離し、以下の写真に示す位置へ貼り付けます。
...
スピーカー上面の小さな穴が左方向になるように取り付けると、スイッチプレートの枠から音が抜けて聞こえやすくなります。
...
```aside
ノブ（つまみ）は、スイッチプレートと基板を組み合わせた後に取り付けます。
```
...
- [打鍵感向上：スイッチプレートと基板間のパテ埋め](#打鍵感向上：スイッチプレートと基板間のパテ埋め)
...
スイッチプレートの4隅に仮固定用のスイッチをはめ込んでから、基板へ取り付けます。  
...
```aside
スイッチプレートを取り付けた後は、以下の基板おもて側の部品の取り付け・取り外しができなくなります。
基板おもて側の部品：インジケータLED，スピーカー，ロータリーエンコーダ，スタビライザー

スイッチプレートを取り外すのが面倒なため、部品の取り付け忘れがないか確認しておきます。
```
...
使用するレイアウトに合わせ、スイッチプレートの4隅にスイッチをはめ込みます。  
ロータリーエンコーダを取り付けている場合は、その隣へはめ込みます。

スイッチの取り付け方向がソケットの方向に合うようにしてください。  
例えば、左上のキーは上下逆向きにはめ込みます。

![スイッチプレート4隅につけたスイッチ](../assets/BuildGuide_v.0.4/_DSF0205.jpeg)
スイッチプレート4隅につけたスイッチ

スイッチがスイッチプレートの枠にぴったり密着してはまっていることを確認します。

![スイッチのスイッチプレートへのはまり具合（異常）](../assets/BuildGuide_v.0.4/_DSF0218.jpeg)
スイッチのスイッチプレートへのはまり具合（異常）

![スイッチのスイッチプレートへのはまり具合（正常）](../assets/BuildGuide_v.0.4/_DSF0219.jpeg)
スイッチのスイッチプレートへのはまり具合（正常）

基板を表側が上になるように置きます。

4隅にスイッチをはめ込んだプレートを基板に乗せ、スイッチの端子とソケットの位置を合わせます。
スタビライザーを取り付けているときは、スイッチプレートの枠にはまるように位置を調整します。

![スイッチプレートの枠にはまったスタビライザー](../assets/BuildGuide_v.0.4/_DSF0212.jpeg)
スイッチプレートの枠にはまったスタビライザー（Wideレイアウトの例）
...
![スイッチプレートを基板に仮固定](../assets/BuildGuide_v.0.4/_DSF0442.jpeg)
スイッチプレートを基板に仮固定
...
取り付けたら、スイッチがスイッチプレートの枠にぴったりとはまっていることを確認します。  
スイッチがスイッチプレートから浮いているような場合は、プレートを指で引っ張り、スイッチと密着させます。
</details>
